...
<button href="#" @click="remove(post)" class="right blue-text" v-if="login.token != null && login.id == post.user._id">Remove</button>
				</div>
			</div>
		</div>
	</div>
	...
	export default{
		...
		created: function(){
			...
			this.$http.get('/api/posts').then(function(response){
				...
				this.showProgress = false
				Materialize.toast('Error: ' +error.status.Text, 3000)			
			})
		}
	}
</script>